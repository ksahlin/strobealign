...
bool is_proper_nam_pair(const Nam nam1, const Nam nam2, float mu, float sigma) {
    ...
    // r1 ---> <---- r2
    bool r1_r2 =
        nam2.is_revcomp && (r1_ref_start <= r2_ref_start) && (r2_ref_start - r1_ref_start < mu + 10 * sigma);

    // r2 ---> <---- r1
    bool r2_r1 =
        nam1.is_revcomp && (r2_ref_start <= r1_ref_start) && (r1_ref_start - r2_ref_start < mu + 10 * sigma);
...
inline std::vector<NamPair> get_best_scoring_nam_pairs(
    ...
    std::sort(nam_pairs.begin(), nam_pairs.end(), [](const NamPair& a, const NamPair& b) -> bool {
...
std::vector<ScoredAlignmentPair> align_paired(
    ...
    if (nams1.empty() && !nams2.empty()) {
        ...
        std::vector<ScoredAlignmentPair> pairs = rescue_read(
            read1, read2, aligner, references, nams2, max_tries, dropoff, swapped_details, k, mu, sigma
        );
    ...
    for (auto& [score_, n1, n2] : nam_pairs) {
        ...
        bool r1_r2 = a2.is_revcomp && (a1.ref_start <= a2.ref_start) &&
                     ((a2.ref_start - a1.ref_start) < mu + 10 * sigma);  // r1 ---> <---- r2
        bool r2_r1 = a1.is_revcomp && (a2.ref_start <= a1.ref_start) &&
...
inline void get_best_map_location(
    ...
    if (score_joint > score_indiv) {  // joint score is better than individual
        ...
        for (auto& [nams, read_len] :
             {std::make_pair(std::cref(nams1), read1_len), std::make_pair(std::cref(nams2), read2_len)}) {
            size_t best_score = 0;
...
std::vector<Nam> get_nams(
    ...
    if (map_param.rescue_level > 1 && (nonrepetitive_hits == 0 || nonrepetitive_fraction < 0.7)) {
        ...
        for (int is_revcomp : {0, 1}) {
            auto [n_rescue_hits_oriented, n_partial_hits_oriented, matches_map] = find_matches_rescue(
...
void align_or_map_paired(
    ...
    if (map_param.output_format != OutputFormat::SAM) {  // PAF or abundance
        ...
        get_best_map_location(
            nams_pair[0], nams_pair[1], isize_est, nam_read1, nam_read2, record1.seq.length(),
            record2.seq.length(), abundances, map_param.output_format == OutputFormat::Abundance
        );
        ...
        auto alignment_pairs = align_paired(
            aligner, nams_pair[0], nams_pair[1], read1, read2, index_parameters.syncmer.k, references,
            details, map_param.dropoff_threshold, isize_est, map_param.max_tries
        );
        ...
        if (alignment_pairs.size() == 1 && alignment_pairs[0].score == -1) {
            ...
            if (is_proper && isize_est.sample_size < 400 &&
            ...
            output_aligned_pairs(
                alignment_pairs, sam, map_param.max_secondary, secondary_dropoff, record1, record2, read1,
                read2, isize_est.mu, isize_est.sigma, details
            );
        }
    }
