...
use crate::maponly::get_paired_alignment;
...
fn get_best_paired_alignment<'a>(
    paired_alignments: &'a [ScoredAlignmentPair],
    alignments1: &'a [Alignment],
    alignments2: &'a [Alignment],
    insert_size_distribution: &mut InsertSizeDistribution,
) -> BestPairedAlignment<'a> {
    if let Some(ScoredAlignmentPair {
        score,
        alignment1,
        alignment2,
...
pub fn deduplicate_scored_pairs(pairs: &mut Vec<ScoredAlignmentPair>) {
    pairs.dedup_by(|a, b| {
