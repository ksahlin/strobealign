...
pub trait SyncmerEncoding {
    ...
    /// Turn a 2-bit nucleotide (as returned by [`SeqAccess::nucleotide_bits`],
    /// where 4 means N/ambiguous) into this encoding's per-base value, or
    /// `None` if the base cannot be encoded and the syncmer must be restarted.
    fn encode_nucleotide(bits: u8) -> Option<u8>;
...
impl SyncmerEncoding for RymerEncoding {
    ...
    fn encode_nucleotide(bits: u8) -> Option<u8> {
        ...
        // S1 (A/G vs C/T) is bit 0 of the 2-bit code, S2 (A/C vs G/T) is bit 1.
        let s1 = RYMER_S1[bits as usize];
        let s2 = RYMER_S2[bits as usize];
        Some(bits | (s1 << 2) | (s2 << 3))
    }
...
pub struct SyncmerIterator<S: SeqAccess, E: SyncmerEncoding> {
    seq: S,
    k: usize,
    s: usize,
    t: usize,
...
impl<S: SeqAccess, E: SyncmerEncoding> SyncmerIterator<S, E> {
    pub fn new(seq: S, k: usize, s: usize, t: usize) -> Self {
        Self::with_ry_len(seq, k, s, t, k)
    }

    pub fn with_ry_len(seq: S, k: usize, s: usize, t: usize, ry_len: usize) -> Self {
        SyncmerIterator {
            seq,
            k,
            s,
...
pub type KmerSyncmerIterator<S> = SyncmerIterator<S, KmerEncoding>;
pub type RymerSyncmerIterator<S> = SyncmerIterator<S, RymerEncoding>;
...
impl<S: SeqAccess, E: SyncmerEncoding> Iterator for SyncmerIterator<S, E> {
    type Item = E::Syncmer;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for i in self.i..self.seq.len() {
            let bits = self.seq.nucleotide_bits(i);
            if let Some(c) = E::encode_nucleotide(bits) {
                // not an "N" base
...
mod test {
    ...
    fn kmer_update() {
        ...
        let seq = "TACGTCAA";
        for ch in seq.chars() {
            if let Some(c) = KmerEncoding::encode_nucleotide(NUCLEOTIDES[ch as usize]) {
    ...
    fn rymer_update() {
        ...
        let seq = "TACGTCAA";
        for ch in seq.chars() {
            if let Some(c) = RymerEncoding::encode_nucleotide(NUCLEOTIDES[ch as usize]) {
    ...
    #[test]
    fn rymer_syncmer_iterator() {
        let seq = "ACGTACGTACGTACGTACGT";
        let parameters = SyncmerParameters::try_new(8, 4).unwrap();

        let mut iterator: RymerSyncmerIterator<&[u8]> =
            SyncmerIterator::new(seq.as_bytes(), parameters.k, parameters.s, parameters.t);
        assert!(iterator.next().is_some());
    }

    #[test]
    fn rymer_same_positions_different_ry_len() {
        let seq = "ACGTACGTACGTACGTACGT";
        let parameters = SyncmerParameters::try_new(8, 4).unwrap();
    ...
    #[test]
    fn mixed_rymer_different_hashes_for_different_ry_len() {
        let seq = "ACGTACGTACGTACGTACGT";
        let parameters = SyncmerParameters::try_new(8, 4).unwrap();
