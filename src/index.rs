    ...
    pub fn entries_in_intervals(
        ...
        if entries.len() < MIN_SEARCHABLE {
            for (i, entry) in entries.iter().enumerate() {
                if intervals_contain(intervals, entry.ref_start()) {
        ...
        while block < entries.len() {
            ...
            for &(interval_start, interval_end) in intervals {
                ...
                while i < block_end && entries[i].ref_start() < interval_end {
                    positions.push(start + i);
                    i += 1;
                }
            }
...
fn advance_to_position(
    ...
    if from >= entries.len() || entries[from].ref_start() >= position {
        return from;
    }
    let mut base = from;
    let mut step = 1usize;
    while base + step < entries.len() && entries[base + step].ref_start() < position {
        base += step;
        step *= 2;
        *probes += 1;
    }
    ...
    *probes += (end - base).ilog2() as usize + 1;
    ...
    base + custom_partition_point(&entries[base..end], |h| h.ref_start() < position)
}
...
mod tests {
    ...
    fn an_entry_inside_an_interval_is_found() {
        ...
        let randstrobes = [10usize, 20, 30, 40]
            .map(|position| RefRandstrobe::new(hash, position, 0))
            .to_vec();
        let index = StrobemerIndex::new(
            ...
            randstrobes,
            vec![0, 4, 4],
            ContigStarts::new(vec![0], 100_000),
        );
        let mut found = vec![];
    ...
    fn a_run_past_the_scan_threshold_is_searched() {
        ...
        let randstrobes: Vec<RefRandstrobe> = (0..80)
            .map(|i| RefRandstrobe::new(hash, i * 3, 0))
            .collect();
        let index = StrobemerIndex::new(
            SeedingParameters::new(300),
            1,
    ...
    fn entries_are_found_in_every_offset_block_of_a_run() {
        ...
        let randstrobes: Vec<RefRandstrobe> = [0u8, 1, 2, 3]
            .into_iter()
            .flat_map(|offset| (0..20).map(move |i| RefRandstrobe::new(hash, i * 2, offset)))
            .collect();
        let index = StrobemerIndex::new(
            SeedingParameters::new(300),
            1,
    ...
    fn a_run_with_more_intervals_than_budget_loses_no_entry() {
        ...
        let randstrobes: Vec<RefRandstrobe> = (0..64)
            .map(|i| RefRandstrobe::new(hash, i * 10, 0))
            .collect();
        let index = StrobemerIndex::new(
            SeedingParameters::new(300),
            1,
    ...
    fn advancing_lands_on_the_first_entry_that_reaches_a_position() {
        let hash = 0x0123_4567_89AB_CD00u64 & REF_RANDSTROBE_HASH_MASK;
        let entries =
            [0usize, 5, 5, 12, 40, 41, 90].map(|position| RefRandstrobe::new(hash, position, 0));
        let mut probes = 0;
