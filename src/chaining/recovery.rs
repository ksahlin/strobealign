...
impl Chainer {
    pub fn recover_anchors(
        ...
        for is_revcomp in 0..2 {
            ...
            anchors[is_revcomp].sort_unstable_by_key(|a| (a.ref_start, a.query_start));
            anchors[is_revcomp].dedup();
        }

        for chain in chains {
            let anchors = &anchors[chain.is_revcomp as usize];
            self.recover_into_chain(chain, anchors, index.k(), &index.contig_starts, read_len);
        }
    }

    fn recover_into_chain(
        &self,
        chain: &mut Chain,
        anchors: &[Anchor],
        k: usize,
        ...
        let candidates = anchors_near(anchors, chain, contig_starts, read_len);
...
fn anchors_near(
    ...
    let reach = read_len;
    // `reach` can run past either end of the chain's contig, and anchors on a
    // neighbouring one are not candidates.
    let first = chain
        .ref_start
        .saturating_sub(reach)
        .max(chain.ref_contig_start);
    let last = (chain.ref_end + reach).min(contig_starts.contig_end(chain.ref_start) - 1);
    let start = anchors.partition_point(|a| a.ref_start < first);
    let end = anchors.partition_point(|a| a.ref_start <= last);
...
mod test {
    ...
    fn an_anchor_on_another_contig_is_left_out() {
        ...
        let starts = ContigStarts::new(vec![0, 100], 1_000_000);
        #[rustfmt::skip]
        let mut chain = Chain {
            id: 0,
            ref_start: 0,
            ...
            is_revcomp: false,
            anchors: vec![
                Anchor { ref_start: 60, query_start: 60 },
                Anchor { ref_start: 30, query_start: 30 },
                Anchor { ref_start:  0, query_start:  0 },
            ],
        };
        #[rustfmt::skip]
        let elsewhere = [
            Anchor { ref_start: 120, query_start: 120 },
            Anchor { ref_start: 120, query_start: 120 },
        ];

        chainer.recover_into_chain(&mut chain, &elsewhere, 20, &starts, 200);
    ...
    fn an_anchor_fills_a_gap_in_a_chain() {
        let chainer = Chainer::new(20, ChainingParameters::default());
        let starts = ContigStarts::new(vec![0], 1_000_000);
        #[rustfmt::skip]
        let mut chain = Chain {
            id: 3,
            ref_start: 0,
        ...
        #[rustfmt::skip]
        let missing = [Anchor { ref_start: 90, query_start: 90 }];

        chainer.recover_into_chain(&mut chain, &missing, 20, &starts, 200);

        #[rustfmt::skip]
        assert_eq!(chain.anchors, vec![
            Anchor { ref_start: 120, query_start: 120 },
            Anchor { ref_start:  90, query_start:  90 },
            Anchor { ref_start:  60, query_start:  60 },
            Anchor { ref_start:  30, query_start:  30 },
            Anchor { ref_start:   0, query_start:   0 },
        ]);
    ...
    fn each_end_of_a_chain_takes_more_than_one_anchor() {
        ...
        let mut chain = Chain {
            ...
            is_revcomp: false,
            anchors: vec![
                Anchor { ref_start: 130, query_start: 130 },
                Anchor { ref_start: 100, query_start: 100 },
            ],
        };
        ...
        let outside = [
            ...
            Anchor { ref_start:  70, query_start:  70 },
            Anchor { ref_start: 160, query_start: 160 },
            Anchor { ref_start: 190, query_start: 190 },
        ];

        chainer.recover_into_chain(&mut chain, &outside, 20, &starts, 200);
    ...
    fn the_best_of_the_candidates_for_a_gap_is_the_one_taken() {
        ...
        let mut chain = Chain {
            ...
            is_revcomp: false,
            anchors: vec![
                Anchor { ref_start: 200, query_start: 200 },
                Anchor { ref_start:   0, query_start:   0 },
            ],
        };
        ...
        #[rustfmt::skip]
        let competing = [
            Anchor { ref_start: 100, query_start: 150 },
            Anchor { ref_start: 110, query_start: 110 },
        ];

        chainer.recover_into_chain(&mut chain, &competing, 20, &starts, 400);
    ...
    fn a_gap_takes_every_anchor_that_raises_the_score() {
        ...
        #[rustfmt::skip]
        let missing = [
            Anchor { ref_start:  50, query_start:  50 },
            Anchor { ref_start: 100, query_start: 100 },
            Anchor { ref_start: 150, query_start: 150 },
        ];

        chainer.recover_into_chain(&mut chain, &missing, 20, &starts, 400);
    ...
    fn an_anchor_that_pays_gives_way_to_a_better_one_behind_it() {
        ...
        #[rustfmt::skip]
        let gap = [
            Anchor { ref_start: 20, query_start: 110 },
            Anchor { ref_start: 30, query_start:  30 },
        ];

        chainer.recover_into_chain(&mut chain, &gap, 20, &starts, 400);
    ...
    fn an_anchor_off_the_diagonal_is_left_out() {
        ...
        #[rustfmt::skip]
        let off_diagonal = [Anchor { ref_start: 75, query_start: 62 }];

        chainer.recover_into_chain(&mut chain, &off_diagonal, 20, &starts, 200);
    ...
    fn a_run_drifting_off_the_diagonal_is_cut_where_it_leaves_the_band() {
        ...
        #[rustfmt::skip]
        let drifting = [
            Anchor { ref_start: 220, query_start: 120 },
            Anchor { ref_start: 340, query_start: 140 },
            Anchor { ref_start: 460, query_start: 160 },
            Anchor { ref_start: 580, query_start: 180 },
            Anchor { ref_start: 700, query_start: 200 },
        ];

        chainer.recover_into_chain(&mut chain, &drifting, 20, &starts, 600);
    ...
    fn an_anchor_going_backwards_in_the_query_is_left_out() {
        ...
        #[rustfmt::skip]
        let backwards = [Anchor { ref_start: 45, query_start: 10 }];

        chainer.recover_into_chain(&mut chain, &backwards, 20, &starts, 200);
    ...
    fn an_anchor_the_chain_already_has_is_left_out() {
        ...
        chainer.recover_into_chain(&mut chain, &known, 20, &starts, 200);
    ...
    fn an_anchor_too_far_from_the_chain_is_left_out() {
        ...
        #[rustfmt::skip]
        let far = [Anchor { ref_start: 100000, query_start: 90 }];

        chainer.recover_into_chain(&mut chain, &far, 20, &starts, 200);
