...
//                    << 0 << "\t" << 0 << "\t" << output_read << "\t" << "*" << "\tNM:i:" << sam_aln.ed << "\n";
    ...
    static inline void get_alignment(nam n, std::vector<unsigned int> &ref_len_map, std::vector<std::string> &ref_seqs, std::string &read, std::string &read_rc, int read_len, alignment &sam_aln, int &best_align_dist, int &best_align_index, int cnt, bool &rc_already_comp, unsigned int &tot_ksw_aligned, unsigned int &tot_all_tried){
        bool aln_did_not_fit = false;
        int ref_diff = n.ref_e - n.ref_s;
        int read_diff = n.query_e - n.query_s;
        int min_diff =  read_diff ^ ((ref_diff ^ read_diff) & -(ref_diff < read_diff));
